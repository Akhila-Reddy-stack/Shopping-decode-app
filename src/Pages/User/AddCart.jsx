...
import { getItemByItemNumber } from '../../Services/UserCart';
...
class AddtoCart extends PureComponent {
	...
	componentDidMount = async () => {
		...
		if (this.props.props.location.state != undefined && this.props.props.location.state) {
			await this.setState({
	...
	deleteItem = async () => {
		...
		this.props.enqueueSnackbar(`${this.state.deleteItemName} is Deleted `, options, 500);
	...
	minusrooms = async (item, index) => {
		...
		this.state.itemsList.map((rr, i) => {
			...
			if (item.Itemnumber === rr.Itemnumber && rr.Count > 1) {
				...
				itemIndexValue[i].Priceyoucharge = rr.Priceyoucharge * itemIndexValue[i].Count;
				itemIndexValue[i].Pricewithtax = rr.Pricewithtax * itemIndexValue[i].Count;
			}
		});
		await this.setState({
	...
	Calculation = async () => {
		...
		this.state.itemsList.map((sample) => {
			...
			TotalPrice = listOfPriceyoucharge.reduce(
				(totalCalories, tt) => totalCalories + parseInt(tt.listOfPriceyoucharge, 10),
				0
			);
			console.log(TotalPrice, 'TotalPrice');
			TotalPricewithTax = listOfPriceyoucharge.reduce(
				(totalCalories, tt) => totalCalories + parseInt(tt.listOfTotalPricewithTax, 10),
				0
			);
		});
              ...
              <NavBar />
						...
						<div className="cartnav">
							<Col md={6} className="cart-details">
								<div class="cartt">
									<i class="fas fa-arrow-circle-left" onClick={this.handleBack}></i>
									<span class="youcart"> YOUR CART</span>
								</div>
								<br />
								<div className="cartconfirm">
									<i class="fas fa-info-circle cartinfo"></i>
									<span class="pdt">Confirm your scanned Product before proceeding checkout</span>
								</div>
								{itemsList?.length === 0 && <div class="youremptycart">Your Cart is Empty</div>}
										...
										<div>
											<Row>
												<div className="row mb-3 px-2 status ">
													...
													<span className="price mar">₹{item.Priceyoucharge}.00</span>
													<span className="mar">
														<s>₹{item.Pricewithtax}.00</s>
													</span>
													<span className="mar off">{`${item.Discount} % OFF`}</span>
												</div>
											</Row>
										</div>
									</div>
